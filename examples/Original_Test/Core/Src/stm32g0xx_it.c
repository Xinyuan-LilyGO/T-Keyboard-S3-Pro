...
uint8_t TIM14_Timed_ms_Flag = 0;
uint8_t TIM14_Timed_10ms_Flag = 0;
uint8_t TIM14_Timed_50ms_Flag = 0;
uint8_t TIM14_Timed_1000ms_Flag = 0;
...
{
    static uint8_t TIM14_Count_10 = 0;
    static uint8_t TIM14_Count_50 = 0;
    static uint16_t TIM14_Count_1000 = 0;

    if (htim->Instance == htim14.Instance)
    {
        TIM14_Timed_ms_Flag = 1;

        TIM14_Count_10++;
        TIM14_Count_50++;
        TIM14_Count_1000++;

        if (TIM14_Count_10 >= 10)
        {
            TIM14_Count_10 = 0;
            TIM14_Timed_10ms_Flag = 1;
        }
        if (TIM14_Count_50 >= 50)
        {
            TIM14_Count_50 = 0;
            TIM14_Timed_50ms_Flag = 1;
        }
        if (TIM14_Count_1000 >= 1000)
        {
            TIM14_Count_1000 = 0;
            TIM14_Timed_1000ms_Flag = 1;
        }
    }
}
...
void HAL_I2C_AddrCallback(I2C_HandleTypeDef *hi2c, uint8_t TransferDirection, uint16_t AddrMatchCode)
{
    if (TransferDirection == I2C_DIRECTION_TRANSMIT) // 主机发送，从机接收
    {
        ...
        if (IIC_Slave_Receive_Buff[0] != 0 || IIC_Slave_Receive_Buff[0] != '\0')
        {
            ...
            // IIC_Slave_Send_Struct.Data = 0; // 如果这里清除之前的数据的话可能会出现问题

            switch (IIC_Slave_Receive_Buff[0]) // 判断命令
            {
            case 0x02: // 读取按键命令
    ...
    if (TransferDirection == I2C_DIRECTION_RECEIVE) // 主机接收，从机发送
    {

/*
 * @Description: 
            Factory test example 2
        All devices should be connected and tested at the same time. After testing the main device, 
    proceed to test the slave device. Please make sure to power on the devices after ensuring 
    they are properly connected. Press the 1st button to retest the current test item, press the 5th 
    button to test the next item, and press the 3rd button to skip all tests.
 * @Author: LILYGO_L
 * @Date: 2023-12-26 16:20:20
 * @LastEditTime: 2024-09-05 13:40:57
 * @License: GPL 3.0
 */
#include <Arduino.h>
...
void Original_Test_1(std::vector<unsigned char> device_id)
{
    Select_Screen_All(device_id, true);
...
void Original_Test_2(std::vector<unsigned char> device_id)
{
    Select_Screen_All(device_id, true);
...
void Original_Test_3(std::vector<unsigned char> device_id)
{
    Select_Screen_All(device_id, true);
...
void Original_Test_4(std::vector<unsigned char> device_id)
{
    Select_Screen_All(device_id, true);
    ...
    for (int i = 0; i <= 10; i++)
    {
        for (int j = 0; j < device_id.size(); j++)
        {
            IIC_Bus->IIC_WriteC8D8(device_id[j],
...
void Original_Test_5(std::vector<unsigned char> device_id)
{
    Select_Screen_All(device_id, true);
...
bool IIC_New_Device_Test_Loop(std::vector<unsigned char> device_id)
{
...
void Print_IIC_New_Device_Added(std::vector<unsigned char> device_id_refresh,
                                std::vector<unsigned char> device_id_find, bool state)
{
    Select_Screen_All(device_id_refresh, true);
    ...
    for (int i = 0; i < device_id_find.size(); i++)
    {
        gfx->setCursor(25, 50 + i * 10);
        gfx->printf("[IIC ID]: 0x");
        if (device_id_find[i] < 16)
        {
            gfx->print("0");
            gfx->print(device_id_find[i], HEX);
        }
        else
        {
            gfx->print(device_id_find[i], HEX);
    ...
    Select_Screen_All(device_id_refresh, false);
}
...
void Print_IIC_Device_Registry_Refresh(std::vector<unsigned char> device_id_refresh,
                                       std::vector<unsigned char> device_id_find)
{
    Select_Screen_All(device_id_refresh, true);
...
void setup()
{
    ...
    IIC_New_Device_Test_Loop(vector_temp);
...
void loop()
{
    ...
    if (IIC_Device_ID_State == true)
    {
        ...
        if (IIC_Device_ID_Registry.size() < IIC_Device_ID_Scan.size()) // 添加设备
        {
            ...
            if (temp_ID[0] == IIC_MAIN_DEVICE_ADDRESS) // 删除主设备
            {
                temp_ID.erase(temp_ID.begin());
            }
            ...
            Print_IIC_Device_Registry_Refresh(IIC_Device_ID_Registry, temp_ID);

            LCD_Initialization(temp_ID); // 初始化所有扫描到的设备

            // 扫描到的所有被添加的设备
            if (IIC_New_Device_Test_Loop(temp_ID) == true)
            {
                Print_IIC_New_Device_Added(IIC_Device_ID_Registry, temp_ID, true);
            }
            else
            {
                Print_IIC_New_Device_Added(IIC_Device_ID_Registry, temp_ID, false);
            }
            delay(1000);

            Print_IIC_Info(temp_ID);
            delay(1000);
        }
